...
const SchoolProfile = () => {
    ...
    const fetchProfileData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3000/api/profile/${schoolName}`
        );
  ...
  const handleUsernameChange = async (e) => {
    if (e.key === "Enter") {
      ...
      try {
        await axios.put(
          `http://localhost:3000/api/profile/updateUsername/${schoolName}`,
          {
  ...
  const handlePasswordChange = async (e) => {
    if (e.key === "Enter") {
      ...
      try {
        await axios.put(
          `http://localhost:3000/api/profile/updatePassword/${schoolName}`,
          {
